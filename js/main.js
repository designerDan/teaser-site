$(document).ready(function() {
	...
	var $sHeader = $('.header'),
		$sGallery = $('#sequence'),
		$sGalleryButtons = $('.sequence-buttons'),
		$sContent = $('.content-secondary');

	//set gallery height
	$sGallery.css('height', function() {

		var $headlineHeight = $sGallery.find('h2').first().outerHeight();

		var $imgHeight = $sGallery.find('img').first().outerHeight();

		var $fCaptionsHeight = $sGallery.find('figcaption').first().outerHeight();

		var $buttonsHeight = $sGallery.find('.sequence-buttons').outerHeight();

		var $total = $headlineHeight+$imgHeight+$fCaptionsHeight+$buttonsHeight;

		$total+=parseInt($(this).css('marginBottom'));

		return $total;

	});

	$sGalleryButtons.css('top', function() {

		return -$sGallery.find('h2').first().outerHeight()-$(this).outerHeight();

	})

	//parallaxing goodness
	$window.scroll(function() {
		
		if($sHeader.outerHeight()>=$window.scrollTop()){
		
			$sHeader.css('top', -$window.scrollTop()/2);

			$sContent.css('top', $window.scrollTop());
		
		}

		//if($sContent.outerHeight())
		
		$sGallery.css('top', $window.scrollTop())/8;

	});

	//keeps gallery size in check
